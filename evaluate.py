...
def run_agents(args):
    ...
    with open(args.annotations_json, "r") as file:
        questions_data = json.load(file)
    all_questions = list(questions_data["questions"])
    
    # If end_index is -1, we rely on num_questions or run to the end
    if args.end_index == -1:
        if args.num_questions != -1:
            end_idx = args.start_index + args.num_questions
        else:
            end_idx = len(all_questions)
    else:
        end_idx = args.end_index

    questions = all_questions[args.start_index : end_idx]
    
    print(f"Running questions from index {args.start_index} to {end_idx} (Total: {len(questions)})")
    # --- MODIFICATION END ---
    ...
    # Generate signatures of API methods.
    safe_sample_count = min(len(questions), args.num_api_questions)
    api_questions = random.sample(questions, safe_sample_count)
    # --- MODIFICATION END ---
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Index to start processing questions from (default: %(default)s)",
    )
    parser.add_argument(
        "--end-index",
        default=-1,
        type=int,
        help="Index to end processing questions (exclusive). Overrides num-questions if set.",
    )
    args = parser.parse_args()
